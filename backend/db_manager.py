...
def backup_db():
    ...
    try:
        ...
        # Create a timestamped backup file
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        backup_path = f"backup_{database}_{timestamp}.sql"
        ...
        try:
            ...
            logger.info(f"Running backup command: {' '.join(command)}")
            ...
            if result.returncode != 0:
                logger.error(f"Error during backup: {result.stderr.decode()}")
                return False
        ...
        logger.error(f"Backup not supported for database type: {db_url}")
        return False
...
def check_db():
    ...
    try:
        ...
        inspector = inspect(engine)
        tables = inspector.get_table_names()
        required_tables = ["chats", "messages", "gemini_files"]
